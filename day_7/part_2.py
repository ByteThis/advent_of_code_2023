...
def carry_j(dictionary):
    j_value = dictionary.pop('J', None)
    max_count_key = max(dictionary, key=dictionary.get, default=None)
    dictionary[max_count_key] += j_value if j_value else 0
...
def get_hand_rank(cards):
    ...
    if cards != 'JJJJJ':
        card_counts = carry_j(card_counts)
